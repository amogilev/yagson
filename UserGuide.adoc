...
* YaGson resolves classes by name using either the context class loader (if present), or a simple `Class.forName()`,
so all serialized classes *MUST* be available in either the context class loader or the current defining class loader;
